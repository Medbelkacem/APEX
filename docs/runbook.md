...
**The live deployment today is free-tier Vercel (web) + Render (API) — see
`docs/DEPLOY-FREE.md` for the actual, current deploy/redeploy steps, DNS
records, and environment variables.** What follows describes the
Docker/Caddy/VPS path this repo also supports, kept working as an
alternative (e.g. if free-tier limits become a problem) but not what's
currently running.

1. CI runs lint → typecheck → test → build on every PR (`.github/workflows/ci.yml`).
2. On merge to `master`, build and push the `docker/api.Dockerfile` and
...
Hostinger is the domain **registrar** only today — nothing is hosted there.
DNS for `apex-dental-solution.com` may be on Hostinger's own nameservers or
delegated to Cloudflare; either way, see `docs/DEPLOY-FREE.md` §1 for the
exact records. (An earlier revision of this doc described running both apps
on Hostinger's hPanel Node.js hosting directly; that plan was superseded by
the current Vercel + Render setup before it was ever deployed.)

If Hostinger (or any single host) is ever used to run both apps directly
again — e.g. a VPS — the `docker/` stack (Caddy fronting both apps on one
origin, so `NEXT_PUBLIC_API_URL` can stay unset and same-origin) is built for
exactly that; see `docker/compose.prod.yml` and the "Production checklist"
above.
